...
def create_complaint_record(payload: CreateComplaintRequest, request: Request) -> CreateComplaintResponse:
...
def log_conversation(payload: ConversationLogRequest, request: Request) -> ConversationLogResponse:
    db = get_request_db(request)
...
def generate_summary(payload: GenerateSummaryRequest, request: Request) -> GenerateSummaryResponse:
    db = get_request_db(request)
    complaint = db.query(Complaint).filter(Complaint.id == payload.complaint_id).first()
    if not complaint:
        raise HTTPException(status_code=404, detail="Complaint not found")
...
def get_complaint(id: int, request: Request) -> ComplaintViewResponse:
    db = get_request_db(request)
